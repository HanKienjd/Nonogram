...
const parseLevel = (category: IDataCategory, level: IDataLevel): ILevel => ({
  ...
  grid: chunkArray(
    decompressFromBase64(level.grid)
      .split("")
      .map((solution: string) => Boolean(parseInt(solution, 2))),
    category.size,
  ),
  title: level.title,
});
...
export const getCategoryNames = (): Array<ICategory["title"]> =>
  categories.map((category) => category.title);

export const getCategory = (title: string): ICategory =>
  parseCategory(findCategoryByTitle(title));

export const getCategoryWithLevels = (
  title: string,
): ICategory & { levels: ILevelSummary[] } => {
...
export const getLevel = (categoryId: string, levelId: string): ILevel => {
  ...
  const category = categories.find(
    (categoryData) => categoryData.title === categoryId,
  ) as IDataCategory;
